...
Original fourstates code developed by Zitong Zhao which created the four-states model and initial PETSc implementation.
"""
...
SCHEMA_VERSION: str = "12223224"  # bump whenever math/outputs change
...
def _monitor_dense(
    ...
    # periodic debug print of layer fractions (no ctx state)
    dbg_dt = 0
    if dbg_dt > 0.0:
        # store next print time on the function object itself
        next_t = getattr(_monitor_dense, "_dbg_next_t", None)
        if next_t is None:
            next_t = t_now  # start printing immediately
        if t_now + 1e-12 >= float(next_t):
            _debug_print_layer_fractions(t_now, u.getArray(readonly=True))
            next_t = float(next_t) + dbg_dt
        setattr(_monitor_dense, "_dbg_next_t", next_t)
...
def _debug_print_layer_fractions(t: float, u_arr: np.ndarray) -> None:
    """
    Print relative hydrogen distribution across layers.
    Percentages are relative to total H in the whole sample (trapped + mobile).
    Also prints within-layer mobile vs trapped split.
    """
    trapped = np.asarray(u_arr[0:5], dtype=float)
    mobile  = np.asarray(u_arr[5:10], dtype=float)
    total_layer = trapped + mobile

    total_all = float(np.sum(total_layer))
    if total_all <= 0.0:
        print(f"\n[H%] t={t/3600:10.4f} hr  total_H={total_all:.6e}")
        return

    layers = ("A", "B", "C", "D", "E")

    print(f"\n[H%] t={t/3600:10.4f} hr  total_H={total_all:.6e}")
    print("  layer | trapped%    mobile%     total%   |  within-layer: mobile% trapped%")
    for i, L in enumerate(layers):
        trap_i = float(trapped[i])
        mob_i  = float(mobile[i])
        tot_i  = float(total_layer[i])

        trap_pct_global = 100.0 * trap_i / total_all
        mob_pct_global  = 100.0 * mob_i  / total_all
        tot_pct_global  = 100.0 * tot_i  / total_all

        if tot_i > 0.0:
            mob_pct_local  = 100.0 * mob_i  / tot_i
            trap_pct_local = 100.0 * trap_i / tot_i
        else:
            mob_pct_local = 0.0
            trap_pct_local = 0.0
        ...
        print(
        f"   {L:>2}   |"
        f" {trap_pct_global:9.3e}"
        f" {mob_pct_global:9.3e}"
        f" {tot_pct_global:9.3e}"
        f" |"
        f" {mob_pct_local:7.3f}"
        f" {trap_pct_local:7.3f}"
    )