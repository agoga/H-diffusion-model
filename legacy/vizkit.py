...
def _plot_layers(
    ...
    *,
    ...
    legend=False,
    delta_t0: bool = False,   # NEW: subtract first value of THIS stage
):
    """
    Unified layer plotter:
    - A, B, C plotted normally
    - D+E shown as a single c-Si layer
    - trapped/mobile also combined for c-Si (D+E)
    - if delta_t0=True, each curve is shifted by its own stage-initial value:
        y(t) -> y(t) - y(t0_stage)
    """
    ...
    colA, colB, colC, colD = ("#1f77b4", "#ff7f0e", "#2ca02c", "#d62728")
    ...
    # Existing behavior: baseline is firing initial value
    # New behavior (delta_t0=True): baseline is stage initial value
    if delta_t0:
        dA = nA - nA[0]
    else:
        tT_A_fire, fT_A_fire = res.total("A", stage="firing")
        dA = nA - fT_A_fire[0]
    ...
    axA.plot(
        xscale(tA), dA, lw=1.8, color=colA,
    ...
    # ======================================================
    # TRAPPED/MOBILE CURVES (optional), with D+E combined for c-Si
    # ======================================================
    if plot_trapped_mobile:
        ...
        nT_A = maybe_delta(nT_A)
        nM_A = maybe_delta(nM_A)

        if neg_a:
            nT_A = -nT_A
            nM_A = -nM_A

        ax.plot(xscale(tT_A), nT_A, color=colA, label="Al₂O₃ trapped",**trapped_style)
        ...
        for L, dx, col, name in [
            ...
            ax.plot(xscale(tT), nT, color=col, label=f"{name} trapped",**trapped_style)
            ax.plot(xscale(tM), nM, ls=ls_mobile,  color=col, label=f"{name} mobile")

        # c-Si trapped/mobile = D+E combined
        ...
        if units == "cm2":
            nT_csi = nT_D * dxD + nT_E * dxE
            nM_csi = nM_D * dxD + nM_E * dxE
        else:
            w = dxD + dxE
            nT_csi = (nT_D * dxD + nT_E * dxE) / w
            nM_csi = (nM_D * dxD + nM_E * dxE) / w

        nT_csi = maybe_delta(nT_csi)
        nM_csi = maybe_delta(nM_csi)

        ax.plot(xscale(tT_D), nT_csi, color=colD, label="c-Si trapped",**trapped_style)
    ...
    if ylog:
        # ax.set_yscale("log")
        ax.set_yscale("symlog", linthresh=1e8, linscale=1.0, base=10)
    if x_lim_left:
        ax.set_xlim(left=x_lim_left)
